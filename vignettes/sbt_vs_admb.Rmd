...
plot(lr$Recruitment)
lines(rp$recruitment_y)

rdev_y <- parameters$par_rdev_y
n_year2 <- n_year - 2
for (y in n_year2:n_year) rdev_y[y] <- lr$AC_penalty[2] * parameters$par_rdev_y[y - 1] + parameters$par_rdev_y[y]
n_year1 <- n_year - 1
for (y in n_year1) rdev_y[y] <- 1.4 * parameters$par_rdev_y[y - 1] + parameters$par_rdev_y[y]
for (y in n_year) rdev_y[y] <- parameters$par_rdev_y[y]
plot(lr$Rdev, xlim = c(85, n_year))
lines(rdev_y, col = 2)

plot(lr$Reps)
lines(parameters$par_rdev_y)
